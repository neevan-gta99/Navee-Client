...
function AddMensWA() {
  ...
  const accessorySubcategories = ["Wallet", "Belt", "Tie", "Sunglasses", "Cufflinks"];
  const watchBrands = ["Rolex", "Casio", "Titan", "Fastrack", "Fossil", "Rado", "Timex", "Seiko"];
  const accessoryBrands = ["Allen Solly", "Louis Philippe", "Tommy Hilfiger", "Levi's", "Peter England"];
  const accessoryMaterials = ["Leather", "Synthetic Leather", "Fabric", "Metal", "Plastic", "Polycarbonate"];
  const beltSizes = ["28", "30", "32", "34", "36", "38", "40", "42"];
  const colors = ["Black", "Brown", "White", "Tan", "Navy", "Gray"];
 ...
 const calculateTotalStock = useCallback(() => {
  ...
  // Conditionally choose the sizes array
  const currentSizes = product_Type === "Accessories" ? beltSizes : [];
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/addProduct/mens-wa`, {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  return (
    <div>
      <h1 className='mt-20'>Add Men's Watches & Accessories</h1>
      <br /><br />
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <input {...register("category", { required: "Category is required" })} placeholder={category ?? ""} value="Mens WA" readOnly type="text" />
        <br /><br />
