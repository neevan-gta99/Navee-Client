...
function AddGirlsWA() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (product_Type === "Accessories" && selectedSizes.length > 0) {
      selectedSizes.forEach((size) => {
    ...
    try {
      const response = await fetch("http://localhost:6173/api/addProduct/girls-wa", {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <br /><br />
        <input {...register("category", { required: "Category is required" })} placeholder={category ?? ""} value="Girls WA" readOnly type="text" />
        <br /><br />
