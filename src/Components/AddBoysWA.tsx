...
function AddBoysWA() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (product_Type === "Accessories" && sub_Category === "Cap" && selectedSizes.length > 0) {
      ...
      // Logic for one-size products (watches, school bags, belts, sunglasses, etc.)
    ...
    try {
      const response = await fetch("http://localhost:6173/api/addProduct/boys-wa", {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <br /><br />
        <input {...register("category", { required: "Category is required" })} placeholder={category ?? ""} value="Boys WA" readOnly type="text" />
        <br /><br />
        ...
        <fieldset>
          ...
          {/* Renders colors and stock options conditionally */}
          {selectedSizes.length > 0 ? (
            // If sizes are selected (e.g., for caps)
            ...
            // If no sizes are selected (e.g., for watches or other accessories)
            <div className="mb-6">
              <div className="grid grid-cols-2 gap-4">
                {colors.map((color) => {
